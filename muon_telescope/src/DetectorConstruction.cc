 ...
 iceMat(0), lvol_ice(0), lvol_ice_top(0), worldMat(0), pvol_world(0), defaultMat(0)
{
  ...
  nbOfLayers          = 2;		    //10
  nbOfModules         = 5;		    //9
  nbDetectors	      = 100;		//12693 per circa 2km 8250 per 1.3km
...
G4VPhysicalVolume* DetectorConstruction::ConstructCalorimeter()
{
  ...
  G4double R = (fiberLength+10)/(2*sin(twopi/(2*nbDetectors)));
  //G4double R = 10*m;
  //mountain
  /*G4EllipticalCone*      
  ...
  */
  ...
  /*sizeX = 1.4*R;                            
  G4Sphere*      
  svol_mountain = new G4Sphere("mountain",			//name
                  0, sizeX, 0*deg, 360*deg, 0*deg, 90*deg);	//size
  ...
  */
  ...
  //
  G4double apotema = (fiberLength+10)/(2*tan(twopi/(2*nbDetectors)));
  G4double apotemaCorretta = apotema + sqrt(2)*calorThickness/2;
  
  G4double alpha = twopi/nbDetectors;
  G4double xx, yy;
  G4RotationMatrix rotm1;
  rotm1.rotateY(45*deg);
  ...
  //MONTAGNA
  new G4PVPlacement(0,
    		      G4ThreeVector(0.,0., 0.4*R+50), //rotation+position
